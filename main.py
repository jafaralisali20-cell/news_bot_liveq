...
def translate(text: str, dest: str) -> str:
    try: return GoogleTranslator(source="auto", target=dest).translate(text) or text
        ...
        try:
            ...
            for entry in feed.entries[:3]:
                ...
                if img_bytes:
                    ...
                    async with session.post(f"{TG_API}/sendMessage", json=payload) as fallback_r: await fallback_r.read()
...
async def health(_): return web.Response(text="Bot Running")
